...
import { coseToJwk, derToRaw, getAlgorithmFromKey, jwtAlgToWebCrypto, parse } from "./parse.js";
import { base64ToJSON, bufferToBase64Url, quoteString, toBuffer } from "./utils.js";
/** @import { AuthenticatorData, AssertionResponse, AttestationResponse, AttestationObject, AssertionObject, AssertionOptions, AttestationOptions, JWK, COSE, PackedAttestation, TPMAttestation, AndroidKeyAttestation, AndroidSafetyNetAttestation, AppleAttestation, CompoundAttestation, FIDO2U2FAttestation } from './types.js' */
...
async function concatNonce(/** @type {Uint8Array} */ rawAuthenticatorData, /** @type {Uint8Array} */ rawClientData) {
	const data = new Uint8Array(rawAuthenticatorData.length + 32);
	data.set(rawAuthenticatorData);
	data.set(new Uint8Array(await crypto.subtle.digest("sha-256", rawClientData)), rawAuthenticatorData.length);
	return data;
}

function coerceCryptoKey(/** @type {COSE | JWK | CryptoKey} */ key) {
	if ("1" in key) {
		key = coseToJwk(key);
	}
	if ("kty" in key) {
		return crypto.subtle.importKey("jwk", key, getAlgorithmFromKey(key), true, ["verify"]);
	}
	return key;
}
...
async function verifyAndroidSafetyNet(
	...
	const algorithm = jwtAlgToWebCrypto(header.alg);
...
export async function assertion(a, opts) {
	...
	if ("kty" in key) {
		...
		key = await crypto.subtle.importKey("jwk", key, algorithm, true, ["verify"]);
	}
	...
	let signature = toBuffer(response.signature, "signature");

	// WebAuthn uses DER-encoded ECDSA signatures, but WebCrypto expects raw R||S format
	if (isEcdsa) {
		const elementLength = { "P-256": 32, "P-384": 48, "P-521": 66 }[algorithm.namedCurve] || 32;
		signature = derToRaw(signature, elementLength);
	}
